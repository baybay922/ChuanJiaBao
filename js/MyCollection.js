...
$("#changeShopCar").on("click",function(e){
    ...
    if ($(this).hasClass("l_done")) {
      ...
      var l_statement = $("<p>").html("<span class='l_choiceAll'></span>全选<button id='Delete'>删除</button>").addClass("l_statement").appendTo("header");
...
function CollectionList(){
  if (dataInt == "") {
    ...
    //插入初始结构
    var l_commodity = $("<ul style='background:transparent;'>").addClass("l_colletionList").appendTo("section");
        ShoppingCarNum = 0;
    ...
    $.each(dataInt.rows,function(key,value){
      ...
      if (value.commodityType == 2 ) {
        ...
        var listBox1 = $("<li style='border:none;margin-bottom:1rem;display:inline-block;background:#fff;'>").appendTo(l_commodity),
          ...
          mainInfo1 = $("<p style='text-align:left; line-height: 100%; position: absolute;bottom: 1.5rem;left: 9.5rem;'>").attr({"id":"commodityNum","data-commodityVal":value.commodityValue}).addClass("text-overflow").text("￥"+parseFloat(value.commodityValue).toLocaleString("en-us",{stylt:"currency",currency:"USD"})+".00").appendTo(listBox1),
          addBTN1 = $("<p style='width: 6rem;height: auto;position: absolute;bottom: 1rem;right: 5%;overflow: hidden;color: #afafaf;text-align: right;'>").attr("id","addBTN").html("<button>立即购买</button>").appendTo(listBox1);
      }
    })
...
function ShopCarListDelete(){
  if (dataInt == "") {
    ...
    //插入初始结构
    var l_commodity = $("<ul style='background:transparent;'>").addClass("l_colletionListDel").appendTo("section"),
        ShoppingCarNum = 0;
